...
ts_length <- dat_all %>% 
  ...
  summarise(
   depl_start = min(start_date), depl_end = max(end_date)
  ) %>% 
  ungroup() %>% 
  mutate(
    ts_length_weeks = difftime(depl_end, depl_start, units = "weeks"),
...
sliderInput(
  ...
  min = 0, max = round(max(ts_length$ts_length_weeks), digits = -1), 
  value = round(min(ts_length$ts_length_weeks), digits = -1), 
...
# logic
# ts_short <- reactive({
#   req(input$min_ts_length)
# 
#   ts_short <- ts_length %>%
#     filter(ts_length_weeks < input$min_ts_length)
# 
#   return(ts_short)
# })
...
dat_station <- reactive({
  ...
  if ("All" %in% input$selected_station) { 
    sel_station <- unique(dat_all$station) 
  } else { sel_station <- input$selected_station }
  
  dat_station <- dat_all %>% 
    filter(county %in% sel_county, station %in% sel_station) %>%
    anti_join(
      ts_short, by = join_by(county, station, sensor_depth_at_low_tide_m)
    ) #%>%
    # anti_join(
...
output$fig_county <- renderPlotly({
  ...
  # apply gap check here again because stations can have data at one depth,
  # then gap, then another depth (e.g., Ram Island)
