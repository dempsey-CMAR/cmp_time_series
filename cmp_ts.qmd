...
The Station and Depth dropdowns will be reset when the inputs above are changed.
I could fix this, BUT then the station and depth options would not update based on the previous selections.
...
output$depth_dropdown <- renderUI({
  selectInput(
    inputId = "selected_depth", "Select Depth(s)", selected = "All",
