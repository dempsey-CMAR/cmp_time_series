...
sliderInput(
  ...
  min = 0, 
  max = round(max(dat_all$ts_length_weeks), digits = -1), 
...
sliderInput(
  inputId = "ts_years",
  "Select Start Year",
  min = min(dat_all$start_year),
  max = max(dat_all$end_year),
  value = min(dat_all$start_year),
 # value = c(min(dat_all$start_year), max(dat_all$end_year)),
  step = 1, width = w, ticks = TRUE, sep = ""
)
...
**Inputs**
  ...
  - Select Start Year: move slider to choose the minimum start year. Time series with a start year greater than or equal to the selected value will be kept.
  ...
  - Select Station(s): choose the station(s) of interest. The options update based on the inputs above.  
  - Select Depth(s): choose the depth(s) of interest. The options update based on the inputs above. 
...
- Station & Depth: chart of when data was collected at each selected station and depth. It is recommended to filter before clicking on this tab to reduce the number of panels. The legend on the right can be used to filter the depths shown on the chart. This will not impact any of the other tabs.
...
dat_county <- reactive({
  ...
  dat_county <- dat_all %>% 
    filter(
      ...
      start_year >= input$ts_years[1],
    #  end_year <= input$ts_years[2],
